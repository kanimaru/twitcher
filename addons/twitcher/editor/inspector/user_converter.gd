...
func reload() -> void:
	...
	if user and new_user_login == user.login and new_user_id == user.id:
		return

	if not user and TwitchEditorSettings.load_current_twitch_user:
		update_user(TwitchEditorSettings.default_user, true)
		return
