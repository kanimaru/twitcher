...
class_name TwitchESCharityCampaignStop
	


## 
## #/components/schemas/CharityCampaignStopEvent
class Event extends TwitchData:
	...
	@export var stopped_at: String:
		set(val): 
			stopped_at = val
			track_data(&"stopped_at", val)
	...
	static func from_json(d: Dictionary) -> Event:
		...
		if d.get("stopped_at", null) != null:
			result.stopped_at = d["stopped_at"]
		return result
