...
func dump_and_convert(path: String, buffer_in: PackedByteArray = [], output = "%s.res" % path, parallel = false) -> SpriteFrames:
	...
	var err: int = thread.start(_do_work.bind(path, buffer, output, mutex))
...
func _create_temp_filename() -> String:
	...
	var uniq: String = _create_unique_key()
...
func _get_frame_delay(file: String) -> Array[int]:
	var out: Array = []
	var glob_path: String = ProjectSettings.globalize_path(file)
	var exit_code: int = OS.execute(imagemagic_path, [ glob_path, "-format", "%T\\n", "info:" ], out)
	if exit_code != 0:
		_log.e("Couldn't get frame delays from %s return code is: %s" % [file, exit_code])
	var frame_delays: Array[int] = []
	for delay: String in out[0].split("\n"):
...
func _extract_images(file: String, target_folder: String) -> bool:
	var out: Array = []
	var glob_file_path: String = ProjectSettings.globalize_path(file)
	var glob_extracted_file_path: String = ProjectSettings.globalize_path(target_folder + "%04d.png")
	var code: int = OS.execute(imagemagic_path, [ glob_file_path, glob_extracted_file_path ], out, true)
	if code != 0:
		_log.e("unable to convert %s: %s" % [file, "\n".join(out)])
		return false
	return true


func _create_fallback_texture() -> SpriteFrames:
	var sprite_frames: SpriteFrames = SpriteFrames.new()
	sprite_frames.add_frame(&"default", fallback_texture)
	return sprite_frames


func _build_frames(folder_path: String, frame_delays: Array[int]) -> SpriteFrames:
	_log.i("Build Frames")
	var frames: PackedStringArray = DirAccess.get_files_at(folder_path)
	if len(frames) == 0:
		return _create_fallback_texture()
	...
	for filepath in frames:
		...
		var delay: float = frame_delays[idx] / 1000.0
		var image: Image = Image.new()
		var error: int = image.load(folder_path + filepath)
		if error != OK:
			return _create_fallback_texture()

		var frame: ImageTexture = ImageTexture.create_from_image(image)
...
func _cleanup(force: bool = false):
	if folder_to_delete.size() % DELETE_COUNT == 0 || force:
		...
		for folder in folder_to_delete:
			var glob_folder: String = ProjectSettings.globalize_path(folder)
